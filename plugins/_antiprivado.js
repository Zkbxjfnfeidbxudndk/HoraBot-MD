...
// تجاهل الرسائل من البوت أو بدون محتوى
if (m.fromMe) return;
if (m.isGroup) return !1;
if (!m.message) return !0;

// استثناءات لبعض الشاتات
if (["120363399336067154@newsletter", "120363399336067154@newsletter"].includes(m.chat)) return;

// استثناء بعض الكلمات المفتاحية
if (m.text.includes('PIEDRA') || m.text.includes('PAPEL') || m.text.includes('TIJERA') || 
    m.text.includes('menu') || m.text.includes('estado') || m.text.includes('code') || 
    m.text.includes('Code') || m.text.includes('bots') || m.text.includes('serbot') || 
    m.text.includes('jadibot') || m.text.includes('reg') || m.text.includes('verificar') || 
    m.text.includes('Serbot') || m.text.includes('Jadibot') || 
    m.text.includes('jadibot code') || m.text.includes('serbot code') || 
    m.text.includes('jadibot --code') || m.text.includes('serbot --code')) return !0;
...
if (setting.antiPrivate && !isOwner && !isROwner) {
    if (user.warnPv) {
        console.log(`[🚫 نظام منع الخاص 🚫]`);
        throw !0;
    }

    if (!user.warnPv) {
        m.reply(`🚫┊مـمـنـوع الـتـحـدث فـي الـخـاص\n🤧┊لــذا ســا يـتـم حـظـرك\n*───── ꒰ა⋟﹏⋞໒꒱ ─────*\n> اذا كـنـت تـريـد فـك الـحـظـر او شـراء الـبـوت او اي شـي\n*───── ꒰ა⋟﹏⋞໒꒱ ─────*\n⚰️┊رقـم الـمـطـور:↶\n\nhttps://wa.me/33760509044‬‏${[nna].getRandom()}`
        );
        user.warnPv = true;
        throw !0;
    }
